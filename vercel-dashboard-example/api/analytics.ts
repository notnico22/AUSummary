...
/**
 * Analytics endpoint that aggregates and returns game statistics
 * Access at: https://your-app.vercel.app/api/analytics
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    // TODO: In production, fetch from your database
    // For now, we'll process the in-memory data

    if (gamesData.length === 0) {
      // Return empty analytics if no data yet
    ...
    const analytics = calculateAnalytics(gamesData);
...
// Export the in-memory storage for the stats endpoint to use
export { gamesData };
